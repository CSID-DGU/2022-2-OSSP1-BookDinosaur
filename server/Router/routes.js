...
router.get("/api/db/users/bookreports/:userid", async (req, res) => {
  ...
  try {
    const data = await pool.query(
      "SELECT R.*, B.title AS bookTitle FROM BOOKWEB.BookReportTB R LEFT JOIN BOOKWEB.BookTB B ON R.isbn = B.isbn WHERE R.userid = ? ORDER BY date DESC",
      [userid]
    );
    console.log(data); // LOGGING
    if (data.length != 0) {
...
router.get("/api/recommend/svd", async (req, res) => {
  ...
  try {
    ...
    var useridList = await pool.query(
      "SELECT userid FROM BOOKWEB.UserTB WHERE NOT userid = ?",
      [req.session.userId]
    );
    var isbnList = await pool.query(
    ...
    for (var i = 0; i < ulen; i++) {
      for (var j = 0; j < ilen; j++) {
        var ratingData = await pool.query(
    ...
    var reportNum = await pool.query(
...
router.get("/api/recommend/cos", async (req, res) => {
  try {
    ...
    try {
      var allUser = await pool.query(
        "SELECT DISTINCT U.userid, U.preference FROM BOOKWEB.UserTB U, BOOKWEB.BookReportTB R WHERE U.userid = R.userid AND NOT U.userid = ?",
        [req.session.userId]
      );
      var userData = allUser;
    } catch {
    ...
    var myPrefer = myData[0].preference.split(",");
